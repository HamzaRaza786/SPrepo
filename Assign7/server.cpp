...
void *read_conn(void *msg){
    ...
    while(true){
        ...
        if(co <= 3){
            ...
            if(token == NULL)
                write(STDOUT_FILENO,"Invalid command\n",16);
            else if(strcheck(token) == false && token != NULL){
            bool exi = false;
            char str1[2000] = "";
            while(token!=NULL){
                    strcat(str1,token);
        ...
        else if(strcheck(token) == true && token != NULL){
            bool exi = false;
                if(token == NULL){
                    write(STDOUT_FILENO,"Invalid command\n",16);
                }
     ...
     pthread_detach(thread2);
        ...
        exit(1);
            ...
            char* s;
               s = div(token,sum);
             if(strcmp(s,"Zero") == 0)
                    write(msgsock,"Divide by Zero Error",20);
            else{
        ...
        int ch = write(msgsock,"Error: Could not run the given file\n",37);
        if(ch == -1){
                perror("Error write in pipe:");
                exit(0);
            }
